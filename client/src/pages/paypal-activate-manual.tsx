...
export default function PayPalActivateManual() {
  ...
  const handleManualActivation = async () => {
    try {
      ...
      const response = await fetch('/api/paypal/manual-activate', {
        method: 'POST',
        headers: {
      ...
      <Card className="w-full max-w-md bg-gradient-to-br from-gray-800/90 to-gray-900/90 border-gray-700/50 backdrop-blur-sm">
        <CardHeader className="text-center">
          ...
          <CardTitle className="text-2xl font-bold text-white">
            Activación Manual PayPal
          </CardTitle>
          
          <CardDescription className="text-gray-300">
            Activa tu cuenta después del pago con PayPal
          </CardDescription>
        </CardHeader>
