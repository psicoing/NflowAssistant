...
export default function EsEnLanguageToggle() {
  ...
  return (
    <div className="flex shrink-0 items-center gap-1 sm:gap-2">
      <div className="flex items-center h-8 sm:h-10 px-0.5 sm:px-1 bg-gradient-to-r from-blue-600/20 to-purple-600/20 border border-blue-500/30 rounded-lg">
        {langOptions.map((lang, index) => (
          <div key={lang.code} className="flex items-center">
            <button
              ...
              className={`flex items-center gap-0.5 sm:gap-1 px-1 sm:px-2 py-1 rounded transition-all duration-200 ${
            ...
            >
              ...
              <span className="text-[10px] sm:text-xs font-medium">{lang.label}</span>
            </button>
            {index < langOptions.length - 1 && (
              <div className="w-px h-3.5 sm:h-4 bg-blue-400/30"></div>
            )}
          </div>
        ))}
      </div>
      ...
      {showAutoDetect && userCountry && (
        <div className="hidden sm:block text-xs text-blue-300/80 animate-pulse">
          🌍 {userCountry}
        </div>
      )}
    </div>
